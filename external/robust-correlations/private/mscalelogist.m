...
% It is iteratively computed. It can resist 50% resultliers. 
...
if n==1 && p==1
    result=0;  %when X is a one by one matrix, all scale estimators must equal to 0
    return
elseif  n==1      
    ...
    if s_0==0
       result(i)=0;
        ...
        while j<=options.k
            ...
            if j > 1
                if abs(lasts-step) < 1e-4 * step % stop if error is less than 0.01% of value
                    break
                end
            end
            lasts=step;
